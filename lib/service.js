...
export class DesktopRuntime {
  ...
  async snapshot(params = {}, { diff = false, observed = true } = {}) {
    ...
    // A query answers with matches and no tree, so it must not become the diff
    // baseline: doing so made the next verification report every element as added.
    const isQuery = result?.query !== undefined
    const previous = isQuery
      ? this.snapshots.get(String(result?.window?.hwnd ?? ''))?.result
    ...
    const structural = diff ? diffSnapshots(previous, result) : undefined
    return {
      result,
      previous,
      diff: structural === undefined ? undefined : { ...structural, truncated: result?.truncated === true },
      elapsedMs,
    }
  }

  /**
   * Whether a verification diff is trustworthy evidence that the action did
   * nothing. A first capture and a capped tree are not: the change may simply be
   * deeper than the caps reached, and calling that "no change" would invite a
   * retry of an action that worked.
   * @param diff - a {@link snapshot} diff, or undefined.
   */
  static diffIsNoOp(diff) {
    if (diff === undefined || diff === null || diff.first === true) return false
    if (diff.truncated === true) return false
  ...
  static renderActionText(result, diff, { prefix = '', windows = undefined, repeat = 0 } = {}) {
    ...
    if (DesktopRuntime.diffIsNoOp(diff) && repeat >= 1) {
      parts.push(
