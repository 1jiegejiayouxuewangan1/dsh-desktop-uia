...
import { elementLabel, renderDiff, windowLabel } from './format.js'
...
export function registerDesktopTools(ctx, { runtime, store, logger }) {
  ...
  register({
    ...
    async execute(args, exec) {
      ...
      const { result } = await runtime.snapshot(params)
      const text = args.query === undefined
        ? DesktopRuntime.renderSnapshotText(result)
        : DesktopRuntime.renderQueryText(result)
  ...
  register({
    ...
    async execute(args, exec) {
      ...
      return { ok: true, text: DesktopRuntime.renderActionText(result, diff) }
    },
  })
