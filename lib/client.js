...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      auditReads: '读取操作也记入日志',
      auditReadsHint: '关闭后只记录写入动作；长时间浏览控件树时，日志不会被读取记录挤满。',
      caps: '快照上限',
      maxDepth: '最大深度',
      maxNodes: '最大节点数',
      refresh: '刷新',
    ...
    const en = {
      ...
      auditReads: 'Log read-only calls too',
      auditReadsHint: 'Turn this off to record writes only, so a long control-tree exploration cannot crowd the log.',
    ...
    const S = {
      ...
      muted: { color: 'var(--dsw-alias-text-secondary, #6b7280)', fontSize: 12.5 },
      ...
      // Fixed layout needs every clipped cell to say so, or a long title pushes the
      // right-hand columns out of the card.
      ellipsis: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
      colProcess: { width: 132 },
      colPid: { width: 62 },
      colState: { width: 96 },
      colAction: { width: 86 },
      colTime: { width: 64 },
      colAct: { width: 142 },
      colOutcome: { width: 76 },
      scroller: { maxHeight: 248, overflow: 'auto' },
      ...
      // Fixed columns per row: ids and types line up down the list, the name is the
      // only part that can be long, and patterns/flags stay in their own columns
      // instead of running into the name.
      treeId: { flex: '0 0 54px', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace', fontSize: 11.5, color: 'var(--dsw-alias-text-secondary, #6b7280)' },
      treeType: { flex: '0 0 112px', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis' },
      treeName: { flex: '1 1 auto', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' },
      treeTail: { flex: '0 0 168px', display: 'flex', gap: 8, alignItems: 'baseline' },
      treePatterns: { flex: '1 1 auto', minWidth: 0, textAlign: 'right', overflow: 'hidden', whiteSpace: 'nowrap' },
      treeFlags: { flex: '0 0 66px', textAlign: 'right', fontStyle: 'italic', fontSize: 12, color: 'var(--dsw-alias-text-secondary, #6b7280)' },
    }
    ...
    function ProcessEditor({ t, label, hint, items, onChange }) {
      ...
      return h('div', { style: { marginTop: 12 } },
        ...
        // The field sits directly under its label, and the current entries read as
        // its result instead of standing between the label and the input.
      ...
      return h('div', { style: { ...S.scroller, maxHeight: 300 } },
        elements.map((element) => {
          ...
          return h('div', {
            ...
            h('span', { style: S.treeId }, element.id),
            h('span', { style: S.treeType, title: element.type }, element.type),
            h('span', { style: S.treeName, title: element.name }, element.name === '' ? null : `"${element.name}"`),
            h('span', { style: S.treeTail },
              h('span', { style: S.treePatterns }, element.patterns.length === 0 ? null : h('span', { style: S.badge }, element.patterns.join(','))),
              h('span', { style: S.treeFlags }, flags.length === 0 ? null : flags.join(' '))))
        }))
    }
      ...
      return h('div', { style: S.scroller },
        h('table', { style: S.table },
          h('thead', null, h('tr', null,
            h('th', { style: { ...S.th, ...S.colProcess } }, t('colProcess')),
            h('th', { style: S.th }, t('colTitle')),
            h('th', { style: { ...S.th, ...S.colPid } }, t('colPid')),
            h('th', { style: { ...S.th, ...S.colState } }, t('colState')),
            h('th', { style: { ...S.th, ...S.colAction } }, t('colOperation')))),
          h('tbody', null, windows.map((window) => {
            ...
            if (window.maximized === true) flags.push(t('maximized'))
            return h('tr', {
              key: String(window.hwnd),
              style: selectedHwnd === window.hwnd ? { background: 'var(--dsw-alias-bg-tertiary, rgba(127,127,127,.16))' } : null,
            },
              h('td', { style: { ...S.td, ...S.colProcess, ...S.ellipsis }, title: String(window.process ?? '') }, h('span', { style: S.mono }, String(window.process ?? '?'))),
              h('td', { style: { ...S.td, ...S.ellipsis, cursor: 'pointer' }, onClick: () => onSelect(window), title: String(window.title ?? '') }, String(window.title ?? '')),
              h('td', { style: { ...S.td, ...S.colPid, ...S.mono } }, String(window.pid)),
              h('td', { style: { ...S.td, ...S.colState, ...S.muted, ...S.ellipsis } }, flags.join(', ')),
              h('td', { style: { ...S.td, ...S.colAction } }, h('button', { type: 'button', style: S.button, disabled: busy, onClick: () => onFocus(window) }, t('focus'))))
          }))))
    }
    ...
    function AuditTable({ t, rows }) {
      ...
      return h('div', { style: S.scroller },
        h('table', { style: S.table },
          h('thead', null, h('tr', null,
            h('th', { style: { ...S.th, ...S.colTime } }, t('colTime')),
            h('th', { style: { ...S.th, ...S.colAct } }, t('colAction')),
            h('th', { style: S.th }, t('colTarget')),
            h('th', { style: { ...S.th, ...S.colOutcome } }, t('colOutcome')))),
          h('tbody', null, rows.map((row, index) => {
            ...
            return h('tr', { key: `${String(row.at)}-${String(index)}` },
              h('td', { style: { ...S.td, ...S.colTime, ...S.mono, ...S.muted } }, String(row.at ?? '').slice(11, 19)),
              h('td', { style: { ...S.td, ...S.colAct, ...S.ellipsis }, title: `${String(row.tool ?? '')} ${String(row.action ?? '')}`.trim() }, `${String(row.tool ?? '')}${row.action === undefined ? '' : ` ${String(row.action)}`}`),
              h('td', { style: { ...S.td, ...S.ellipsis }, title: `${String(row.target ?? '')}${row.targetWindow === undefined ? '' : ` — ${String(row.targetWindow)}`}` }, String(row.target ?? '')),
              h('td', { style: { ...S.td, ...S.colOutcome, color: outcomeColor } }, String(row.outcome ?? '')))
          }))))
    }
      ...
      return h('div', { style: S.wrap },
        ...
        h('div', { style: S.card },
          ...
          approval === null ? h('div', { style: S.muted }, t('loading')) : h('div', null,
            ...
            h('label', { style: { ...S.radio, marginTop: 8 } },
              h('input', {
                type: 'checkbox',
                checked: settings.behavior.auditReads !== false,
                disabled: busy,
                onChange: (event) => void saveSettings({ behavior: { auditReads: event.target.checked } }),
              }),
              h('span', null,
                h('div', null, t('auditReads')),
                h('div', { style: S.muted }, t('auditReadsHint')))),
